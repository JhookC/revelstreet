...
export function RouteCard({ route }: Props) {
  ...
  return (
    <Link
      ...
      className="block rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-5 shadow-sm transition hover:border-accent hover:ring-2 hover:ring-accent/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-medium uppercase tracking-wide text-[var(--color-content-muted)]">
            {route.operatorId}
          </p>
          <p className="mt-0.5 text-lg font-semibold text-[var(--color-content)]">
            {route.id}
          </p>
        </div>
        <span
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="tabular-nums text-[var(--color-content-muted)]">{pct}%</span>
        </div>
      ...
      {current && status !== 'complete' && (
        <p className="mt-3 truncate text-sm text-[var(--color-content-soft)]">
          {activeStop ? 'At: ' : 'Next: '}
          <span className="font-medium text-[var(--color-content)]">{current.label}</span>
        </p>
      )}
    </Link>
  );
